...
At most 2000 calls will be made to all functions combined.
    ...
    public class DesignAuthenticationManager
    {
        ...
        private Dictionary<string, int> tokenExpiry;

        public DesignAuthenticationManager(int timeToLive)
        {
            this.timeToLive = timeToLive;
            this.tokenExpiry = new Dictionary<string, int>();
        }
        ...
        {
            if (tokenExpiry.ContainsKey(tokenId) && tokenExpiry[tokenId] > currentTime)
            {
                tokenExpiry[tokenId] = currentTime + timeToLive;
            }
            // else: expired or doesn't exist — do nothing
        }

        public int CountUnexpiredTokens(int currentTime)
        {
            int count = 0;
            foreach (var kvp in tokenExpiry)
            {
